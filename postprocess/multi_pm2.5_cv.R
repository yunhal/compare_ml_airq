...
pm_ap <- read.csv(paste(plotdir_base,'daily_pm25_AIRPACT.csv',sep = ''))

pm_ap$X <- as.Date(pm_ap$X)
colnames(pm_ap)[c(1:3)] <- c('date','AIRPACT','AQI_AIRPACT')
...
for (device in devices){
    for (timelen in timelens){
        ...
        alldt <- merge(alldt,pm_ap,, by=c('date','site'))
        colnames(alldt)[c(5,6)] <- c('twoRF','AQI_twoRF') #avoid using leading number
        #'rename twoRF twoRF *' can change filenames in linux
        ...
        for (m in models){
            ...
            ggplot(alldt)+
              ...
              #ggtitle("(a) AP")+
              xlab('Obs. 24h PM2.5 (μg/m3)')+
              ylab(paste(m,'24h PM2.5 (μg/m3)'))+
              ...
              theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                    panel.background = element_blank(), axis.line = element_line(colour = "black"),
                    panel.border = element_rect(colour = "black", fill=NA, size=1))+
              annotate("text", x = 0.15, y=200, size = 6, hjust=0, col='red',
                       label = paste("R2 : ",r2,"\nNMB: ",nmb, "%\nNME: ",nme, "%", sep=''))
        ...
        for (m in models){
          ...
          taylor.diagram(alldt[,'obs'],alldt[,'obs'],add=FALSE,col="black",pch=1,pos.cor=TRUE,
                         ...
                         pcex=1,cex.axis=1,normalize=TRUE,mar=c(4,3,4,3),xlim=4)
          ...
          for (s in unique(alldt$site)){
            taylor.diagram(alldt[alldt$site==s,'obs'],alldt[alldt$site==s,'twoRF'],add=TRUE,col="green",pch=1,pos.cor=TRUE,
                           ...
                           show.gamma=TRUE,ngamma=3,gamma.col=8,sd.arcs=0,
                           ref.sd=FALSE,sd.method="sample",grad.corr.lines=c(0.2,0.4,0.6,0.8,0.9),
                           pcex=1,cex.axis=1,normalize=TRUE,mar=c(4,3,4,3),xlim=3)
        ...
        for (m in models){
          a <- ggmap(test) +
            ...
            #for pm2.5
            scale_size_continuous("Obs. Daily PM2.5 Mean\n  (μg/m3)",
                                  limits=c(0,15),breaks=c(1,2,5,10),range = c(1,5)
                                  #range = c(2,4)
            )+
            #NMBs
            # using jet colors
        ...
        for (m in models){
          a <- ggmap(test) +
            ...
            scale_fill_gradientn("NMB (%)",colours = jet.colors0(7), #c("#00007F", "#002AFF", "#00D4FF", "#FFFFFF"), #
                                 breaks=seq(-70, 70, by=20),limits=c(-70,70), oob=squish) +
            #breaks=ceiling(seq(-100, -30, by=20)),limits=c(-100,-30), oob=squish) +
            #scale_shape_manual("Location Setting",values = c(21, 22, 24))+
            # ordering what legent you want to show first and second
            guides( size = guide_legend(order = 1, size =7),
        ...
        write.csv(summary_obs,paste(outdir_base,'PM2.5_obs_',timelen,'_',device,'.csv',sep=''),row.names = FALSE)
        ...
        write.csv(stat,paste(outdir_base,'PM2.5_stats_',timelen,'_',device,'.csv',sep=''),
                  row.names = TRUE)
    }
}
